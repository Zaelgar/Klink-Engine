...
namespace
{
	DXGI_FORMAT GetFormats(RenderTarget::Format format)
	{
		...
		case RenderTarget::Format::RGBA_F32:	return DXGI_FORMAT_R32G32B32A32_FLOAT;
...
void RenderTarget::BeginRender(Klink::Graphics::Colours::Colour colour)
{
	float ClearColor[4] = { colour.r, colour.g, colour.b, colour.a }; // RGBA
...
{
	GetDeviceContext()->PSSetShaderResources(index, 1, &mShaderResourceView);
}

void RenderTarget::UnbindPS(uint32_t index)
{
	static ID3D11ShaderResourceView* dummy = nullptr;
	GetDeviceContext()->PSSetShaderResources(index, 1, &dummy);
}