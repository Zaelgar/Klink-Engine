...
Sampler::~Sampler()
{
	ASSERT(mSampler == nullptr, "[Sampler] Sampler must be released before destruction");
}

void Sampler::Initialize(Filter filter, AddressMode addressMode)
{
	D3D11_FILTER d3dFilter = GetFilter(filter);
	D3D11_TEXTURE_ADDRESS_MODE  d3dAddresssMode = GetAddressMode(addressMode);

	D3D11_SAMPLER_DESC sampDesc{};
	sampDesc.Filter = d3dFilter;
	sampDesc.AddressU = d3dAddresssMode;
	sampDesc.AddressV = d3dAddresssMode;
	sampDesc.AddressW = d3dAddresssMode;
...
{
	GetDeviceContext()->PSSetSamplers(slot, 0, &mSampler);
}